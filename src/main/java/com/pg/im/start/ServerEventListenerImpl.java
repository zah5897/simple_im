 ...
 */
package com.pg.im.start;

import com.pg.im.start.event.MsgEvent;
import com.pg.im.start.event.UserEvent;
import com.pg.im.start.helper.LocalServiceHelper;
import io.netty.channel.Channel;
import com.pg.im.start.helper.EventQueueManager;
import net.openmob.mobileimsdk.server.event.*;
...
public class ServerEventListenerImpl implements ServerEventListener {
    ...
    public int onVerifyUserCallBack(String userId, String token, String extra, Channel session) {
        ...
        int code = LocalServiceHelper.login(userId, token, session.remoteAddress().toString());
        //事件进入队列处理
       // EventQueueManager.get().putEvent(new VerifyUserEvent(userId, token, extra, session.remoteAddress().toString()));
        return code;
    }
    ...
    public void onUserLoginAction_CallBack(String userId, String extra, Channel session) {
        ...
        EventQueueManager.get().putUserEvent(new UserEvent(userId,extra,session.remoteAddress().toString(),0));
    ...
    public void onUserLogoutAction_CallBack(String userId, Object obj, Channel session) {
        ...
        EventQueueManager.get().putUserEvent(new UserEvent(userId,obj!=null?obj.toString():null,session.remoteAddress().toString(),-1));
    }
    ...
    public void onTransBuffer_C2C_CallBack(String userId, String from_user_id,
        ...
        MsgEvent msgEvent=new MsgEvent(userId);
        msgEvent.setFrom_user_id(from_user_id);
        msgEvent.setFingerPrint(fingerPrint);
        msgEvent.setTypeu(typeu);
        msgEvent.setOffline(false);
        msgEvent.setSendTime(new Date());
        msgEvent.setDataContent(dataContent);
        EventQueueManager.get().putMsgEvent(msgEvent);
    }
